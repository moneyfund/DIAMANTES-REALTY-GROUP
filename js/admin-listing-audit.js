(function initAdminListingAudit() {
  const auditState = { properties: new Map(), unsubscribe: null, observer: null, started: false, frame: null };
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .admin-listing-audit-row dt{color:#9f1118!important}
      .admin-listing-audit-row dd{font-weight:700!important}
      ...
      .admin-listing-audit-card h3{margin:0 0 8px;color:#8f0007}
      .admin-listing-audit-card p{margin:5px 0;line-height:1.45}
      .admin-listing-audit-card small{display:block;margin-top:8px;color:#667085}
    `;
    document.head.appendChild(style);
  }

  function getAudit(property = {}) {
    if (property.uploadedOnBehalf !== true) return null;
    return {
      uploaderName: property.uploadedByAgentName || property.uploadedByAgentEmail || 'Agente no identificado',
      uploaderEmail: property.uploadedByAgentEmail || '',
      ownerName: property.uploadedOnBehalfOfAgentName || property.agentName || 'Agente no identificado',
      ownerEmail: property.uploadedOnBehalfOfAgentEmail || property.agentEmail || ''
    };
  }

  function propertyForId(id = '') {
    return auditState.properties.get(String(id || '')) || null;
  }
  ...
  function decorateReviewCards() {
    document.querySelectorAll('#reviewList .review-card').forEach((card) => {
      ...
      const audit = getAudit(propertyForId(propertyId));
      ...
      if (!row) {
        ...
        row.className = 'admin-listing-audit-row';
        dl.appendChild(row);
      }
  ...
  function decorateInventoryTable() {
    document.querySelectorAll('#propertyList tr').forEach((row) => {
      ...
      if (!note) {
        ...
        note.className = 'admin-listing-audit-note';
        agentCell.appendChild(note);
      }
  ...
  function decorateReviewModal() {
    ...
    const propertyId = modal.querySelector('[data-review-approve]')?.dataset.reviewApprove
      || modal.querySelector('[data-review-reject]')?.dataset.reviewReject;
    const audit = getAudit(propertyForId(propertyId));
    ...
    if (!card) {
      ...
      card.className = 'admin-listing-audit-card';
    ...
    card.innerHTML = `
      ...
      <small>Dato interno de administración. Esta trazabilidad no se muestra en la ficha pública ni en el panel del agente propietario.</small>
    `;
  }
  ...
  function decorate() {
    injectStyles();
    decorateReviewCards();
    decorateInventoryTable();
    decorateReviewModal();
  }

  function scheduleDecorate() {
    if (auditState.frame !== null) return;
    auditState.frame = requestAnimationFrame(() => {
      auditState.frame = null;
      decorate();
    });
  }

  function start() {
    if (auditState.started || !window.inmoFirebase?.db) return false;
    auditState.started = true;
    auditState.unsubscribe = window.inmoFirebase.db.collection('properties').onSnapshot((snapshot) => {
      auditState.properties = new Map(snapshot.docs.map((entry) => [entry.id, { id: entry.id, ...entry.data() }]));
      scheduleDecorate();
    }, (error) => console.warn('[AdminListingAudit] No se pudo cargar la trazabilidad.', error));
  ...
  if (!start()) {
    ...
    const retry = window.setInterval(() => {
      if (start() || Date.now() - startedAt > 15000) window.clearInterval(retry);
    }, 350);
  }
})();
